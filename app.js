...
function normalizeCategory(category) {
  ...
  if (value.includes("外套") || value.includes("衣")) {
    return "外套";
  }
...
function parseNeed() {
  ...
  if (budgetMatch) {
    need.budget = Number(budgetMatch[1]);
  }

  /* 天数 */

  const durationMatch = userText.match(/(\d+)\s*天/);

  if (durationMatch) {
    need.duration = durationMatch[1] + "天";
  }
  ...
  const foundDestination = destinations.find(function(item) {
    return userText.includes(item);
  });

  if (foundDestination) {
    need.destination = foundDestination;
  }

  /* 场景 */

  if (userText.includes("旅行") || userText.includes("旅游")) {
    need.scene = "旅行";
  }

  if (userText.includes("通勤")) {
    need.scene = "通勤";
  }

  if (userText.includes("露营")) {
    need.scene = "露营";
  }
  ...
  if (categories.length) {
    need.categories = [...new Set(categories)];
  }
  ...
  if (constraints.length) {
    need.hardConstraints = [...new Set(constraints)];
  }
  ...
  if (existing.length) {
    need.existingItems = [...new Set(existing)];
  }
...
function updateNeedCard() {
  ...
  const ncBudget = document.getElementById("ncBudget");

  if (ncBudget) {
    ncBudget.textContent = formatMoney(userNeed.budget);
  }

  const ncDest = document.getElementById("ncDest");

  if (ncDest) {
    ncDest.textContent = userNeed.destination || "未指定";
  }

  const ncScene = document.getElementById("ncScene");

  if (ncScene) {
    ncScene.textContent = userNeed.scene || "未指定";
  }

  const ncDuration = document.getElementById("ncDuration");

  if (ncDuration) {
    ncDuration.textContent = userNeed.duration || "";
  }
...
function renderTagField(elementId, values, field) {
  ...
  const html = list
    .map(function(value, index) {
      return `
        <span class="inline-tag-wrap">

          <span
            class="item-tag editable-tag inline-edit-tag"
            contenteditable="true"
            spellcheck="false"
            data-inline-array="${escapeHtml(field)}"
          ...
          <button
            type="button"
            class="tag-remove"
            data-remove-field="${escapeHtml(field)}"
  ...
  container.innerHTML =
    ...
    `
      <button
        type="button"
        class="add-tag"
        data-add-field="${escapeHtml(field)}"
      >
        + 添加
      </button>
    `;
...
function saveInlineField(element) {
  ...
  const field = element.getAttribute("data-inline-field");
  ...
  if (field === "budget") {
    ...
    element.textContent = formatMoney(budget);
  }
...
function saveInlineTag(element) {
  if (!element) {
    return;
  }

  const field =
    element.getAttribute("data-inline-array");

  const index = Number(
    element.getAttribute("data-inline-index")
  );
  ...
  if (!value) {
    element.textContent =
      userNeed[field][index] || "";
    return;
  }

  const duplicate = userNeed[field].some(
    function(item, i) {
  ...
  const oldValue = userNeed[field][index];

  userNeed[field][index] = value;
...
function addNeedTag(field) {
  ...
  if (!Array.isArray(userNeed[field])) {
    userNeed[field] = [];
  }

  if (!userNeed[field].includes(text)) {
...
function removeNeedTag(field, index) {
  if (!Array.isArray(userNeed[field])) {
    return;
  }
  ...
  if (
    ...
    targetIndex < 0 ||
    targetIndex >= userNeed[field].length
  ) {
    return;
  }

  const removed = userNeed[field][targetIndex];

  userNeed[field].splice(targetIndex, 1);
...
function productMatchesConstraints(
  ...
  const rules = Array.isArray(constraints)
    ? constraints
    : [];
  ...
  return rules.every(function(rule) {
    ...
    if (value.includes("轻")) {
      return product.tags.includes("轻便");
    }

    if (value.includes("防晒")) {
      return (
    ...
    if (
      ...
      return product.tags.includes("防雨");
    }
...
function getCandidatesForCategory(category) {
  ...
  let candidates = catalog.filter(function(product) {
    return product.cat === normalized;
  });
...
function buildPlanByStrategy(strategy) {
  const result = [];
  ...
  categories.forEach(function(category) {
    const candidates =
      getCandidatesForCategory(category);
...
function makePlan(type) {
  return buildPlanByStrategy(type);
}
...
function generatePlans() {
  ...
  const homePlans =
    document.getElementById("homePlans");
  ...
  log(
    "用户确认需求；规则引擎根据当前需求生成 A/B/C 三套方案",
    "RULE"
  );
...
document.addEventListener(
  ...
  function(event) {
    const element =
      event.target.closest(
        "[data-inline-field], [data-inline-array]"
      );
